...
## ۲. دیپلوی خودکار نسخه‌بندی‌شده با GitHub Actions (بر اساس Tag)

گردش کار اکشن‌های گیت‌هاب در مسیر `.github/workflows/deploy.yml` تنظیم شده است. با پوش هر تگ جدید که با `v` شروع شود (مانند `v1.0.0`)، پروژه به‌طور خودکار بیلد شده و روی Cloudflare Workers دیپلوی می‌گردد.
